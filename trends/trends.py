...
def extract_words(text):
    ...
    for letter in text:
        if letter in ascii_letters:
            ...
            modified += ' '
    return modified.split()  # You may wish to change this line.
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    sentiment_list = [ sentiment_value(get_word_sentiment(word)) for word in tweet_words(tweet) if has_sentiment(get_word_sentiment(word)) ]
    if sentiment_list:
...
def find_state_center(polygons):
    ...
    s_lat = s_lat/s_area
    s_lon = s_lon/s_area
    return make_position(s_lat,s_lon)
